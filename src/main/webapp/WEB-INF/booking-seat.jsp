            ...
            <div class="w-full max-w-4xl bg-white rounded-2xl shadow-xl p-8">
                ...
                <div class="flex items-center gap-3 mb-6 border-b pb-4">
                    <a href="${pageContext.request.contextPath}/booking/cinema_showtime_room"
                        ...
                        <span
                            class="material-symbols-outlined text-2xl group-hover/back:-translate-x-1 transition-transform">arrow_back</span>
                    </a>
                ...
                <c:if test="${not empty error_msg}">
                    <div
                        class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-xl mb-6 flex items-center gap-2">
                ...
                <form action="${pageContext.request.contextPath}/booking/seat" method="POST">
                    <div class="mb-10 max-w-3xl mx-auto flex flex-col gap-3 items-center">
                        <c:forEach var="row" items="${seat_list.stream().map(s -> s.seatRow).distinct().toList()}">
                            <div class="flex items-center gap-2">
                                <span class="w-6 text-center font-bold text-gray-700">${row}</span>

                                <c:forEach var="seat" items="${seat_list}">
                                    <c:if test="${seat.seatRow == row && seat.seatNumber <= 10}">
                                        ...
                                        <c:set var="isVip"
                                            value="${seat.seatType == 'VIP' || seat.seatType == 'vip'}" />

                                        <label class="cursor-pointer">
                                            <input type="checkbox" name="seat_id_list" value="${seat.seatId}"
                                                class="hidden peer" ${!isAvailable && !isSelected ? 'disabled' : '' }
                                                ${isSelected ? 'checked' : '' } />

                                            <div
                                                class="w-10 h-10 rounded-t-lg rounded-b-sm border-2 flex items-center justify-center text-xs font-bold transition-all duration-200
                    ...
                    <div>
                        <a href="${pageContext.request.contextPath}/"
                            ...
                            <span
                                class="material-symbols-outlined text-xl group-hover/cancel:rotate-90 transition-transform duration-300 font-bold">
                                close
                            </span>
                            Hủy bỏ
                        </a>
