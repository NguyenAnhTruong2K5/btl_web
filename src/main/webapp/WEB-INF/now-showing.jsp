<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
    <%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
        ...
        <head>
            ...
            <title>Phim đang chiếu | CinemaVN Scarlet</title>
            ...
            <style>
                ...
                .cinematic-shadow {
                    box-shadow: 0 12px 40px rgba(45, 47, 47, 0.06);
                }
        ...
        <body class="bg-surface font-body text-on-surface antialiased">
            ...
            <main class="pt-28 pb-20 max-w-[1440px] mx-auto px-6 md:px-12">
                <!-- Editorial Header Section -->
                <header class="mb-12">
                    <h1 class="font-headline font-black text-6xl md:text-7xl text-on-surface tracking-tighter mb-4">
                        Phim
                        đang
                        chiếu</h1>
                    <p class="text-on-surface-variant max-w-2xl text-lg leading-relaxed">Khám phá những siêu phẩm
                        điện
                        ảnh
                        mới
                        nhất đang bùng nổ tại hệ thống rạp CinemaVN Scarlet. Trải nghiệm công nghệ trình chiếu đẳng
                        cấp
                        thế
                        giới.</p>
                </header>
                <!-- Dynamic Filter Bar -->
                <!-- <section class="mb-12 bg-surface-container-low p-6 rounded-2xl flex flex-wrap items-center gap-6">
                    <div class="flex-1 min-w-[200px]">
                        <label class="block text-xs font-bold uppercase tracking-widest text-outline mb-2 ml-1">Thể
                            loại</label>
                        <select
                            class="w-full bg-surface-container-lowest border-none rounded-xl py-3 px-4 text-sm font-medium focus:ring-2 focus:ring-primary/10">
                    ...
                    <div class="flex-1 min-w-[200px]">
                        <label class="block text-xs font-bold uppercase tracking-widest text-outline mb-2 ml-1">Quốc
                            gia</label>
                        <select
                            class="w-full bg-surface-container-lowest border-none rounded-xl py-3 px-4 text-sm font-medium focus:ring-2 focus:ring-primary/10">
                    ...
                    <div class="flex-1 min-w-[200px]">
                        <label class="block text-xs font-bold uppercase tracking-widest text-outline mb-2 ml-1">Định
                            dạng</label>
                        <select
                            class="w-full bg-surface-container-lowest border-none rounded-xl py-3 px-4 text-sm font-medium focus:ring-2 focus:ring-primary/10">
                    ...
                    <div class="flex items-end h-full pt-6">
                        <button
                            class="bg-on-surface text-surface py-3 px-8 rounded-xl font-bold text-sm transition-all hover:bg-primary">Lọc
                            kết quả</button>
                    </div>
                </section> -->
                <!-- Movie Grid -->
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-16">
                    <c:if test="${empty nowShowingMovies}">
                        <div class="text-center p-12 text-on-surface-variant col-span-1 sm:col-span-2 lg:col-span-3">
                            Hiện không có phim đang chiếu trong database (status = "Đang chiếu").
                        </div>
                    </c:if>
                        ...
                        <div class="group flex flex-col">
                            <div
                                ...
                                <div class="absolute top-4 left-4">
                                    <span
                                        class="bg-primary text-on-primary text-[10px] font-black px-2 py-1 rounded tracking-tighter uppercase">Đang
                                        chiếu</span>
                                </div>
                                <div
                            ...
                            <div class="px-2">
                                <div class="flex justify-between items-start mb-2">
                                    <h3
                                        class="font-headline font-bold text-xl leading-tight group-hover:text-primary transition-colors">
                                        ${movie.title}</h3>
                                    <div class="flex items-center text-primary font-bold text-sm">
                                        <span class="material-symbols-outlined text-[16px] mr-1"
                                            style="font-variation-settings: 'FILL' 1;">star</span>
                                        <c:out value="${movie.duration != null ? movie.duration : 0}" />
                                    </div>
                                </div>
                                <div class="flex flex-wrap gap-2 text-on-surface-variant text-xs font-medium mb-4">
                                    <span>${movie.description}</span>
                                    <span>•</span>
                                    <span>${movie.duration} phút</span>
                                    <span>•</span>
                                    <span class="text-primary-dim">${movie.releaseDate}</span>
                                </div>
                            </div>
                        </div>
                    </c:forEach>
